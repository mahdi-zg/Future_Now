...
export class ProfileService {
  ...
  getUserProfile(userId: number): Observable<any> {
    return this.http.get(`${BASE_URL}/${userId}`, { headers: this.createAuthorizationHeader() });
  }

  // 🔹 Mettre à jour les informations utilisateur
  updateUserProfile(userId: number, updatedData: any): Observable<any> {
    return this.http.put(`${BASE_URL}/${userId}`, updatedData, { headers: this.createAuthorizationHeader() });
  }
    ...
    deleteProject(projectId: number): Observable<any> {
      return this.http.delete(`${BASE_URL}/deleteProject/${projectId}`, {
        headers: this.createAuthorizationHeader(),
        responseType: 'json' // ✅ Angular attend un JSON
      });
    }
    ...
    addCardsToProject(projectId: number, cards: any[]): Observable<any> {
      return this.http.post(`http://localhost:8080/api/cards/project/${projectId}`, cards, {
        headers: this.createAuthorizationHeader()
      });
    }
...
getCardsByProject(projectId: number): Observable<any[]> {
  return this.http.get<any[]>(`http://localhost:8080/api/cards/project/${projectId}`, {
    headers: this.createAuthorizationHeader()
  });
}
...
uploadCard(projectId: number, formData: FormData): Observable<any> {
  return this.http.post(`http://localhost:8080/api/cards/upload/${projectId}`, formData, {
    headers: this.createAuthorizationHeader()
  });
}
