...
@Component({
  selector: 'app-kiosk-ui',
  templateUrl: './kiosk-ui.component.html',
  styleUrls: ['./kiosk-ui.component.css']
})
export class KioskUiComponent implements AfterViewInit, OnDestroy, OnChanges {
@ViewChild('messagesEnd') messagesEndRef!: ElementRef;
  ...
  if (this.projectId) {
    this.profileService.getProjectById(this.projectId).subscribe(project => {
      this.selectedAvatar = project.logo; // ⚠️ vérifie que ce champ existe
      this.backgroundColor = project.backgroundColor || '#0a1f44';
    });
  }
}
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['projectId'] && !changes['projectId'].firstChange) {
      ...
      if (!projectId) {
        console.log("🚫 Aucun projectId reçu — mode création activé.");
        this.messages = [];
  
        // 🧹 Supprime visuellement l'avatar (efface le canvas)
        ...
        return; // ⛔ Stop ici, pas d'appel à l'API
      }
    ...
    if (changes['visibleCards']) {
      if (this.visibleCards.length > 0) {
        ...
        this.startCardCarousel(); // 🔁 Redémarre l’animation
      } else {
...
toggleFullScreen() {
  ...
  const container = document.querySelector('.avatar-chat-container');
