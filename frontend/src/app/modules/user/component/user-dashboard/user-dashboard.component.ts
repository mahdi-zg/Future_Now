...
export class UserDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe((params) => {
      ...
      if (projectId) {
        this.loadProjectDetails(projectId); // 👈 un seul appel ici suffit
      }
    });
  
    // NE PAS faire un autre loadProjectDetails ici
  }
...
isParameter1Active: boolean = false;
  ...
  validateAppearance() {
    ...
    this.saveAppearance(); // tu as déjà cette méthode
  }
  validatePersonality() {
  ...
  confirmAddCard() {
    ...
    this.cards = [...this.cards]; // 👈 TRÈS IMPORTANT : force la détection de changement
    this.closeCardModal();
  }
...
saveCards() {
  ...
  const newCardsOnly = this.cards
    .filter(card => !card.id) // 👈 Garde uniquement les cartes sans ID (nouvelles)
    .map(card => ({
      title: card.title,
      prompt: card.prompt,
      tags: card.tags,
  ...
  this.profileService.addCardsToProject(this.selectedProjectId, newCardsOnly).subscribe({
    next: () => {
      console.log("✅ Nouvelles cartes sauvegardées !");
    },
    error: (err) => {
      console.error("❌ Erreur lors de la sauvegarde des cartes :", err);
    }
  });
}
...
uploadCardImage(event: any) {
  ...
  if (file) {
    ...
    reader.onload = (e: any) => {
      if (this.newCard) {
        this.newCard.image = 'assets/cards/' + file.name;
...
saveVoiceAndSoul() {
  ...
  this.switchTab('brain'); // Passe à l’étape suivante
}
...
savePersonality() {
  ...
  this.switchTab('lightbulb'); // Passe à l'étape suivante
}
...
updateProject() {
  ...
  this.profileService.updateProject(this.selectedProjectId, updatedProjectData).subscribe({
    next: (res) => {
      ...
      // ✅ Appel ici pour sauvegarder les cartes associées
      this.saveCards();
...
saveStepAndGoNext(stepName: string, nextStep: string) {
  ...
  this.updateProject(); // 🔁 Appelle updateProject

  console.log(`✅ Étape "${stepName}" enregistrée avec succès.`); // <-- ici ton log

  Swal.fire({
    title: `${stepName} updated!`,
    text: `${stepName} section saved successfully.`,
    icon: 'success',
    toast: true,
    position: 'top-end',
    timer: 2000,
