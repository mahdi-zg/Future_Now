...
export class SidebarComponent implements AfterViewInit {
  ...
  selectProject(projectId: number) {
    this.selectedProjectId = projectId; // ✅ Sauvegarde du projet sélectionné
  
    const selected = this.projects.find(p => p.id === projectId);
    console.log("🎯 Projet sélectionné :", selected?.name, "| Couleur :", selected?.backgroundColor);
    ...
    this.profileService.getProjectsByUser(userId).subscribe({
      next: (data) => {
        ...
        .map((proj: any) => ({
          ...
          backgroundColor: proj.colorBackground || '#0a1f44' // ✅ par défaut si vide
        }))
